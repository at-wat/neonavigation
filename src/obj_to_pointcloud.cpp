...
public:
	obj_to_pointcloud():
		...
		engine(seed_gen())
	{
		pubCloud = n.advertise<sensor_msgs::PointCloud2>("clouds", 1, true);

		n.param("frame_id", frame_id, std::string("map"));
		n.param("obj", file, std::string(""));
		if(file.compare("") == 0)
		{
		...
		n.param("points_per_meter_sq", ppmsq, 100.0);

		auto pc = convert_obj(file);
		pubCloud.publish(pc);
	}
private:
	ros::NodeHandle n;
	...
	sensor_msgs::PointCloud2 convert_obj(const std::string &file)
	{
		...
		pcl::toROSMsg(*pc_rs, pc_msg);
		pc_msg.header.frame_id = frame_id;
		pc_msg.header.stamp = ros::Time::now();
		ROS_INFO("pointcloud (%d points) has been generated from %s with %d verticles", 
				(int)pc_rs->size(),
